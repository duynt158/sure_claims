...
export async function POST(req: Request) {
  ...
  const { firstName, lastName, email, phone, restoreCount, referrer }: ContactFormState = await req.json();
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      <body>
        ...
        <p>Restoration projects per month: ${restoreCount}</p>
    ...
    return NextResponse.json({ restoreCount: 'Email sent successfully' });
  } catch (error) {
    console.error(error);
